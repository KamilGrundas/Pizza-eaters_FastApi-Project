...
from src.services.auth_new import get_current_user
...
from src.services.auth_new import create_access_token, create_refresh_token, verify_password
...
async def get_picture(
    ...
    current_user: Optional[User] = Depends(get_current_user),  # Dodawanie opcjonalnego użytkownika
):
    ...
    if request.method == "POST" and comment_text:
        ...
        if current_user:
            await comments.add_new_comment(picture_id, CommentBase(text=comment_text), db,current_user)
        else:
            # Możesz tutaj obsłużyć sytuację, gdy użytkownik nie jest zalogowany
            # na przykład przekierowując na stronę logowania lub wyświetlając komunikat
            pass
    return templates.TemplateResponse(
