from fastapi import HTTPException, Depends, status, BackgroundTasks, Request, APIRouter, FastAPI
...
async def signup(
    ...
    body.password = auth_service.get_hash_password(body.password)
...
async def request_email(body: RequestEmail, background_tasks: BackgroundTasks, request: Request,
    ...
    if user.confirmed:
        return {"message": "Your email is already confirmed", "status_code": status.HTTP_200_OK}
    if user:
        background_tasks.add_task(send_verification_email, user.email, user.username, request.base_url)
